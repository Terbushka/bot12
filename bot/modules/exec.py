...
async def clear(_, message):
    ...
    # Також очищаємо DOWNLOAD_DIR якщо немає активних задач
    try:
        from .. import task_dict, task_dict_lock, DOWNLOAD_DIR
        ...
        if active == 0:
            disk_before = disk_usage(DOWNLOAD_DIR)
            await clean_all()
            disk_after = disk_usage(DOWNLOAD_DIR)
            freed = disk_before.used - disk_after.used
            freed_str = f"{freed / 1024**3:.2f} GB" if freed > 0 else "0 B"
            disk_pct = disk_after.percent
            await send(
                f"✅ Locals Cleared.\n"
                f"🗑 Downloads directory cleaned.\n"
                f"💾 Freed: {freed_str} | Disk: {disk_pct:.1f}% used",
                message,
            )
        else:
            await send(
                f"⚠️ Locals Cleared.\n"
                f"🚫 Downloads NOT cleaned — {active} active task(s) running.",
                message,
            )
    except Exception as e:
        await send(f"Locals Cleared. (disk cleanup error: {e})", message)
