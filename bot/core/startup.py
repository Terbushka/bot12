...
async def update_nzb_options():
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            LOGGER.info("SABnzbd options loaded successfully")
            return
        except Exception as e:
            LOGGER.warning(f"SABnzbd options fetch attempt {attempt}/{max_retries} failed: {e}")
            if attempt < max_retries:
                await sleep(0.5 * attempt)
    LOGGER.error("Failed to fetch SABnzbd options after 5 attempts — SABnzbd may not be available")
...
async def update_variables():
    if (
        not Config.LEECH_SPLIT_SIZE
        or Config.LEECH_SPLIT_SIZE > TgClient.MAX_SPLIT_SIZE
        or Config.LEECH_SPLIT_SIZE <= 2097152000  # встановлено старий non-premium дефолт
    ):
        Config.LEECH_SPLIT_SIZE = TgClient.MAX_SPLIT_SIZE  # автоматично 4 GiB при преміумі

    # Якщо є преміум-акаунт — вмикаємо USER_TRANSMISSION і HYBRID_LEECH автоматично
    ...
    if not await aiopath.exists(".netrc"):
        try:
            async with aiopen(".netrc", "w"):
                pass
            LOGGER.info("Created empty .netrc file")
        except Exception as e:
            LOGGER.warning(f"Could not create .netrc (non-fatal): {e}")

    shell_parts = []
    if await aiopath.exists(".netrc"):
        shell_parts.append("chmod 600 .netrc && cp .netrc /root/.netrc")
    if await aiopath.exists("aria-nox-nzb.sh"):
        shell_parts.append("chmod +x aria-nox-nzb.sh && ./aria-nox-nzb.sh")
    if shell_parts:
        await (await create_subprocess_shell(" && ".join(shell_parts))).wait()
    else:
        LOGGER.info("Skipping .netrc/aria-nox-nzb.sh setup (files not present)")
