...
class TelegramUploader:
    def __init__(self, listener, path):
        ...
        self._dump_chat_id = None  # дамп-канал для тихого бекапу
    ...
    async def _send_media_group(self, subkey, key, msgs):
        ...
        # Зберігаємо лише якщо це не ЛС-доставка
        if self._listener.is_super_chat or (
            self._listener.up_dest and self._listener.up_dest != self._listener.user_id
        ):
            for m in msgs_list:
                self._msgs_dict[m.link] = m.caption
        else:
            # Для PM — просто рахуємо файли за підписом
            for m in msgs_list:
                self._msgs_dict[str(m.id)] = m.caption or ""
