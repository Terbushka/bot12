        ...
        if d["status"] == "finished":
            ...
            if self.is_playlist:
                ...
                if d.get("total_bytes"):
                    ...
                    # Перевірка ліміту 12 ГБ (sync-контекст — async_to_sync)
                    if self._listener.is_leech and self._listener.size > 12 * 1024 ** 3:
                        size_gb = self._listener.size / 1024 ** 3
                        self._on_download_error(
                            f"Вы превысили разрешенный лимит в 12 ГБ\n"
                            f"Размер файла: {size_gb:.2f} ГБ"
                        )
