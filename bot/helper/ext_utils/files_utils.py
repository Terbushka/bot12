...
async def auto_disk_cleanup(active_task_count: int):
    """
    Якщо диск завантажено на ≥80% І немає активних задач — виконати clean_all().
    Безпечно: не чіпає файли задач, що виконуються.
    """
    try:
        usage = disk_usage(DOWNLOAD_DIR)
        percent = usage.percent
        if percent >= 80 and active_task_count == 0:
            LOGGER.warning(
                f"Disk usage {percent:.1f}% ≥ 80% and no active tasks — running auto cleanup"
            )
            await clean_all()
            LOGGER.info("Auto disk cleanup completed")
        elif percent >= 80:
            LOGGER.warning(
                f"Disk usage {percent:.1f}% ≥ 80% but {active_task_count} task(s) still active — skipping cleanup"
            )
    except Exception as e:
        LOGGER.error(f"auto_disk_cleanup error: {e}")
